...
function removeVoidElements(par) {
  var voidElemLen = VoidTags.length;
  for (var b = 0; b < voidElemLen; b++) {  
    if (par.getElementsByTagName(VoidTags[b]).length > 0) {
      var elTags = par.getElementsByTagName(VoidTags[b]);
      for (var o = elTags.length-1; o >= 0;o--) {
	elTags[o].parentNode.removeChild(elTags[o]);
      }
    }
  }	
}
...
function removeSCRIPTs(par) {
  var scr = par.getElementsByTagName('script');
  var scrLen = scr.length; 
  for (var g = scrLen-1; g >= 0; g--) {
    scr[g].parentNode.removeChild(scr[g]);
  }
}
...
function StartProgress(scrsource) {
  ...
  document.getElementById('fff').innerHTML = scrsource;
  removeSCRIPTs(document.getElementById('fff'));      // removes all script tags
  removeHTMLComments(document.getElementById('fff')); // removes all comments
  removeVoidElements(document.getElementById('fff')); // removes all voidElements
  scrsource = document.getElementById('fff').innerHTML;
  ...
  AddIdentifier(objeHt);                     // ---> this function will add a unique identifier to every element.It will be usefull for javascript embed onmouseOver function of the output window.
  setByUserInput=objeHt.innerHTML;
  Explode(document.getElementById('fff'));  // this function adds x and y coordinates based on inheritance. These coordinates will be used for the svg creation.
...
function Explode(el, k) {
  ...
  if (el.children.length > 0) {  
    ...
    for (var i = 0; i < el.children.length; i++) {   
      ...
      Explode(el.children[i], parseInt(elem.getAttribute('JsTreeDepthY')));                            
      rrr = rrr - 1;     
