   ...
   var self = this;
	 ...
	 this.ExplodeTree = function( Dom_Tree ){
                                ...
                                function Explode( el , k ){
                                        ...
                                        if(el.children.length > 0){  
                                                 ...
                                                 /*|*/     if( lastElo.length > 0 ){
												 /*|*/		    
                                                 /*|*/          for( var efh =0; efh < lastElo.length; efh++ ){ //break;
                                                 ...
                                                 /*|*/               
                                                 /*|*/               if(sumIt>TempNumber){
												 /*|*/                   if(sumIt>k){ TempNumber = sumIt;  console.log("sumIt");                         }
												 /*|*/                   else       { TempNumber = sumIt+Math.abs(sumIt-k);console.log("SSSSSSUUMMM");   }
												 /*|*/               }
												                     if(el.id=="rules"){
                                                 ...
                                                 /*|*/
                                                  ...
                                                  for(var i=0;i<el.children.length;i++) {   
                                                      ...
                                                      if(i==0){ elem.setAttribute("JsTreeDepthY",parseInt(elem.parentNode.getAttribute('JsTreeDepthY'),10)+i);  }
                                                      else{     elem.setAttribute("JsTreeDepthY",parseInt(el.children[i-1].getAttribute('JsTreeDepthY'),10)+1); }
	 ...
	 this.TreeInfo    = function(){
                ...
                for(var ts = 2; ts < self.MaxR+1; ts++ ){ 
                             ...
                             if( ts == self.MaxR ){ checkTag=""; checkText="checked"; }
	 ...
	 this.CreateSvg   = function( Dom , foCol , foSiz , sceCol , ratio ){
      ...
      var c3 = document.createElementNS("http://www.w3.org/2000/svg", "rect");
         ...
         //--document.getElementsByTagName("body")[0].appendChild( SvgEl );
	 ...
	 this.GraphOptions = function(){
		 ...
		 var fontColorField = document.getElementById("fontcolor"),
			 ...
			 if ( fontColorField.value.length > 1 ){ fc = fontColorField.value; } else { fc = self.Graph.defaults.text_color;     }
			 if ( BoneColorField.value.length > 1 ){ bc = BoneColorField.value; } else { bc = self.Graph.defaults.skeleton_color; }
			 if ( rationField.value.length    > 1 ){ ra = rationField.value;    } else { ra = self.Graph.defaults.ratio;          }
			 
			 fs = self.Graph.defaults.font_size;
			 
			 document.getElementById('graphContent').innerHTML = "";
			 if( !self.iFrameDom || self.iFrameDom.getElementsByTagName("body")[0].children.length === 0 ){alert("no data.Fill TextArea ,press 'Start' and then 'CreateGraph'");return;}
		     self.CreateSvg( self.iFrameDom.getElementsByTagName("body")[0] , fc , fs , bc , ra );
		 var setByUserInput	 = self.iFrameDom.getElementsByTagName('body')[0].innerHTML;
